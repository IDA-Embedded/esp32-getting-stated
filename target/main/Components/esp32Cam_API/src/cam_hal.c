...
#include <string.h>
#ifdef __ESP32__
#include "../include/cam_hal.h"
#include "../include/ll_cam.h"
			...
			case CAM_STATE_READ_BUF: {
				...
				if(cam_event == CAM_IN_SUC_EOF_EVENT)
				{
					...
					if(cam_obj->jpeg_mode && cnt == 0 &&
					   cam_verify_jpeg_soi(frame_buffer_event->buf, frame_buffer_event->len) != 0)
					{
						ll_cam_stop(cam_obj);
						cam_obj->state = CAM_STATE_IDLE;
					}
				...
				else if(cam_event == CAM_VSYNC_EVENT)
				{
					...
					if(cnt || !cam_obj->jpeg_mode || cam_obj->psram_mode)
					{
						...
						// send frame
						if(!cam_obj->frames[frame_pos].en &&
						   xQueueSend(cam_obj->frame_buffer_queue, (void*)&frame_buffer_event, 0) !=
							   pdTRUE)
						{
...
static esp_err_t cam_dma_config(const camera_config_t* config)
{
	...
	cam_obj->dma_node_cnt =
		(cam_obj->dma_buffer_size) / cam_obj->dma_node_buffer_size; // Number of DMA nodes
	cam_obj->frame_copy_cnt =
		cam_obj->recv_size /
		cam_obj->dma_half_buffer_size; // Number of interrupted copies, ping-pong copy

	ESP_LOGI(
		TAG,
		"buffer_size: %d, half_buffer_size: %d, node_buffer_size: %d, node_cnt: %d, total_cnt: %d",
		cam_obj->dma_buffer_size, cam_obj->dma_half_buffer_size, cam_obj->dma_node_buffer_size,
		cam_obj->dma_node_cnt, cam_obj->frame_copy_cnt);

	cam_obj->dma_buffer = NULL;
	cam_obj->dma = NULL;

	cam_obj->frames = (cam_frame_t*)heap_caps_calloc(1, cam_obj->frame_cnt * sizeof(cam_frame_t),
													 MALLOC_CAP_DEFAULT);
	...
	for(int x = 0; x < cam_obj->frame_cnt; x++)
	{
		...
		if(cam_obj->psram_mode)
		{
			...
			ESP_LOGI(TAG, "Frame[%d]: Offset: %u, Addr: 0x%08X", x, cam_obj->frames[x].fb_offset,
					 (uint32_t)cam_obj->frames[x].fb.buf);
			cam_obj->frames[x].dma = allocate_dma_descriptors(
				cam_obj->dma_node_cnt, cam_obj->dma_node_buffer_size, cam_obj->frames[x].fb.buf);
	...
	{
		cam_obj->dma_buffer =
			(uint8_t*)heap_caps_malloc(cam_obj->dma_buffer_size * sizeof(uint8_t), MALLOC_CAP_DMA);
		if(NULL == cam_obj->dma_buffer)
		{
			ESP_LOGE(
				TAG,
				"%s(%d): DMA buffer %d Byte malloc failed, the current largest free block:%d Byte",
		...
		cam_obj->dma = allocate_dma_descriptors(cam_obj->dma_node_cnt,
												cam_obj->dma_node_buffer_size, cam_obj->dma_buffer);
...
esp_err_t cam_config(const camera_config_t* config, framesize_t frame_size, uint16_t sensor_pid)
{
	...
	ret = ll_cam_set_sample_mode(cam_obj, (pixformat_t)config->pixel_format, config->xclk_freq_hz,
								 sensor_pid);
	...
	xTaskCreatePinnedToCore(cam_task, "cam_task", 2048, NULL, configMAX_PRIORITIES - 2,
							&cam_obj->task_handle, 0);
#elif CONFIG_CAMERA_CORE1
	xTaskCreatePinnedToCore(cam_task, "cam_task", 2048, NULL, configMAX_PRIORITIES - 2,
							&cam_obj->task_handle, 1);
#else
...
camera_fb_t* cam_take(TickType_t timeout)
{
	...
	if(dma_buffer)
	{
		...
		else if(cam_obj->psram_mode && cam_obj->in_bytes_per_pixel != cam_obj->fb_bytes_per_pixel)
		{
			...
			dma_buffer->len =
				ll_cam_memcpy(cam_obj, dma_buffer->buf, dma_buffer->buf, dma_buffer->len);
		}
		return dma_buffer;
	}
	else
