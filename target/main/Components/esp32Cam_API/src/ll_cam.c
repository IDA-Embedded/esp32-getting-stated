...
static size_t IRAM_ATTR ll_cam_dma_filter_grayscale_highspeed(uint8_t* dst, const uint8_t* src,
															  size_t len)
{
	const dma_elem_t* dma_el = (const dma_elem_t*)src;
	size_t elements = len / sizeof(dma_elem_t);
...
static size_t IRAM_ATTR ll_cam_dma_filter_yuyv_highspeed(uint8_t* dst, const uint8_t* src,
														 size_t len)
{
	const dma_elem_t* dma_el = (const dma_elem_t*)src;
	size_t elements = len / sizeof(dma_elem_t);
...
{
	return esp_intr_alloc(ETS_I2S0_INTR_SOURCE, ESP_INTR_FLAG_LOWMED | ESP_INTR_FLAG_IRAM,
						  ll_cam_dma_isr, cam, &cam->cam_intr_handle);
}
...
static bool ll_cam_calc_rgb_dma(cam_obj_t* cam)
{
	...
	ESP_LOGI(
		TAG,
		"node_size: %4u, nodes_per_line: %u, lines_per_node: %u, dma_half_buffer_min: %5u, "
...
esp_err_t ll_cam_set_sample_mode(cam_obj_t* cam, pixformat_t pix_format, uint32_t xclk_freq_hz,
								 uint16_t sensor_pid)
{
