...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div,
					   int pll_pre_div, bool pll_root_2x, int pll_seld5, bool pclk_manual,
					   int pclk_div)
{
	...
	ESP_LOGI(TAG, "Calculated VCO: %d Hz, PLLCLK: %d Hz, SYSCLK: %d Hz, PCLK: %d Hz", VCO * 1000,
			 PLLCLK, SYSCLK, PCLK);
	return SYSCLK;
}

static int set_pll(sensor_t* sensor, bool bypass, uint8_t multiplier, uint8_t sys_div,
				   uint8_t pre_div, bool root_2x, uint8_t seld5, bool pclk_manual, uint8_t pclk_div)
{
	int ret = 0;
	...
	calc_sysclk(sensor->xclk_freq_hz, bypass, multiplier, sys_div, pre_div, root_2x, seld5,
	...
	if(ret == 0)
	{
		ret = write_reg(sensor->slv_addr, SC_PLLS_CTRL3,
						(pre_div & 0x3) << 4 | seld5 | (root_2x ? 0x40 : 0x00));
	}
	if(ret == 0)
	{
...
static int set_image_options(sensor_t* sensor)
{
	...
	ESP_LOGD(TAG,
			 "Set Image Options: Compression: %u, Binning: %u, V-Flip: %u, H-Mirror: %u, Reg-4514: "
			 "0x%02x",
			 sensor->pixformat == PIXFORMAT_JPEG, sensor->status.binning, sensor->status.vflip,
			 sensor->status.hmirror, reg4514);
	return ret;
}
...
static int set_framesize(sensor_t* sensor, framesize_t framesize)
{
	...
	if(sensor->status.binning)
	{
		ret = write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H, settings.total_x,
							 (settings.total_y / 2) + 1) ||
			  write_addr_reg(sensor->slv_addr, X_OFFSET_H, 8, 2);
	}
	else
	{
		ret =
			write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H, settings.total_x, settings.total_y) ||
...
static int set_sharpness(sensor_t* sensor, int level)
{
	...
	ret = write_reg_bits(sensor->slv_addr, 0x5308, 0x40, false) // 0x40 means auto
		  ...
		  write_reg(sensor->slv_addr, 0x5309, 0x10) || write_reg(sensor->slv_addr, 0x530a, 0x10) ||
		  write_reg(sensor->slv_addr, 0x530b, 0x04) || write_reg(sensor->slv_addr, 0x530c, 0x06);
...
{
	return set_pll(sensor, bypass > 0, multiplier, sys_div, pre_div, root_2x > 0, seld5,
				   pclk_manual > 0, pclk_div);
}
...
static int init_status(sensor_t* sensor)
{
	...
	sensor->status.agc =
		!check_reg_mask(sensor->slv_addr, AEC_PK_MANUAL, AEC_PK_MANUAL_AGC_MANUALEN);
	sensor->status.aec =
		!check_reg_mask(sensor->slv_addr, AEC_PK_MANUAL, AEC_PK_MANUAL_AEC_MANUALEN);
	sensor->status.hmirror =
		check_reg_mask(sensor->slv_addr, TIMING_TC_REG21, TIMING_TC_REG21_HMIRROR);
