...
static esp_err_t camera_probe(const camera_config_t* config, camera_model_t* out_camera_model)
{
	*out_camera_model = CAMERA_NONE;
	 ...
	 * Attention: Some sensors have the same SCCB address. Therefore, several attempts may be made
	...
	ESP_LOGI(TAG, "Camera PID=0x%02x VER=0x%02x MIDL=0x%02x MIDH=0x%02x", id->PID, id->VER,
			 id->MIDH, id->MIDL);
...
esp_err_t esp_camera_init(const camera_config_t* config)
{
	...
	{
		ESP_LOGW(TAG, "The frame size exceeds the maximum for this sensor, it will be forced to "
